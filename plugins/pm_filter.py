        ...
        if manual == False:
            ...
            try:
                if settings.get('auto_ffilter'):
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>Searching  {message.text} 🔍</i></b>")
        ...
        try:
            ...
            if not link:
                ...
                if PM_SEARCH == True:
                    ...
                    await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/Strangerthing50>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/Strangerthing50")]]))
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/Strangerthing50")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @kingvj01
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://t.me/CharlieWatson1")
        ],[
        ...
        buttons = [[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('ᴇxᴛʀᴀ', callback_data='EXTRAMOD_TXT')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="https://t.me/CharlieWatson1")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>𝐻𝑒𝑟𝑒 𝐼 𝐹𝑜𝑢𝑛𝑑 𝐹𝑜𝑟 𝑌𝑜𝑢𝑟 𝑆𝑒𝑎𝑟𝑐ℎ 🔎 {search}\n\n</b>"
        else:
            cap = f"<b>𝐻𝑒𝑟𝑒 𝐼 𝐹𝑜𝑢𝑛𝑑 𝐹𝑜𝑟 𝑌𝑜𝑢𝑟 𝑆𝑒𝑎𝑟𝑐ℎ 🔎 {search}\n\n</b>"
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>⚡ 𝐀𝐜𝐭𝐢𝐯𝐞 𝐒𝐮𝐩𝐩𝐞𝐫 𝐒𝐩𝐞𝐥𝐥𝐢𝐧𝐠 𝐂𝐡𝐞𝐤 ⚡</i></b>")
        movienamelist = []
